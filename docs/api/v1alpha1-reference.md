...
func (s *UserStatus) SetPhase(phase UserPhase)
    SetPhase sets the canonical Phase and mirrors it onto the deprecated
    OldPhase field, so readers of either `.status.phase` or the legacy
    `.status.Phase` key observe the same value.
